...
class ManualNode30(Node):
    def __init__(self):
        super().__init__("manual_node_30")
        self.publisher = self.create_publisher(ManualControl, "/mavros/manual_control/send", 10)
        frequency = get_node_frequency("manual_node_30")
        self.timer = self.create_timer(frequency, self.timer_callback)
        self.get_logger().info(f"Manual Node 30 started at {int(1/frequency):,}Hz")
