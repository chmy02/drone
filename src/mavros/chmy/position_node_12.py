...
class PositionNode12(Node):
    def __init__(self):
        super().__init__("position_node_12")
        self.publisher = self.create_publisher(PoseStamped, "/mavros/setpoint_position/local", 10)
        frequency = get_node_frequency("position_node_12")
        self.timer = self.create_timer(frequency, self.timer_callback)
        self.get_logger().info(f"Position Node 12 started at {int(1/frequency):,}Hz")
    ...
    def timer_callback(self):
        ...
        msg.pose.position.y = -0.40000000000000036
