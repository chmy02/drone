...
class PositionNode21(Node):
    def __init__(self):
        super().__init__("position_node_21")
        self.publisher = self.create_publisher(PoseStamped, "/mavros/setpoint_position/local", 10)
        frequency = get_node_frequency("position_node_21")
        self.timer = self.create_timer(frequency, self.timer_callback)
        self.get_logger().info(f"Position Node 21 started at {int(1/frequency):,}Hz")
    ...
    def timer_callback(self):
        ...
        msg.pose.position.x = -5.7
        msg.pose.position.y = -3.8
