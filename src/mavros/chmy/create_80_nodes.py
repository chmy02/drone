...
class PositionNode{node_num}(Node):
    ...
    def timer_callback(self):
        ...
        msg.pose.position.x = {node_num * 0.3 - 12.0}
        msg.pose.position.y = {node_num * 0.2 - 8.0}
...
class ManualNode{node_num}(Node):
    ...
    def timer_callback(self):
        msg = ManualControl()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.x = {(node_num % 5 - 2) * 0.15}
        msg.y = {((node_num + 1) % 5 - 2) * 0.15}
        msg.z = {0.4 + (node_num % 4) * 0.1}
        msg.r = {(node_num % 3 - 1) * 0.08}
        msg.buttons = {node_num % 256}
...
def main():
    ...
    # 위치 명령 노드 16개 추가 (17~32)
    print("\n📍 위치 명령 노드 16개 추가 중...")
    for i in range(17, 33):
        create_position_node(i)
    
    # 수동 조종 노드 16개 추가 (17~32)
    print("\n🎮 수동 조종 노드 16개 추가 중...")
    for i in range(17, 33):
        create_manual_node(i)
    
    # RC 오버라이드 노드 8개 추가 (9~16)
    print("\n🎛️ RC 오버라이드 노드 8개 추가 중...")
    for i in range(9, 17):
        create_override_node(i)
    
    print("\n🎯 총 80개 노드 생성이 완료되었습니다!")
