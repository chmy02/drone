...
def analyze_log_file(log_file):
    ...
    try:
        ...
        df = pd.read_csv(log_file, names=['node_id', 'msg_counter', 
        ...
        # 컬럼 이름 매핑 (기존 분석 코드와의 호환성)
        df['communication_latency_us'] = df['t1_t3_us']      # t1→t3
        df['processing_latency_us'] = df['t3_t4_us']         # t3→t4
        df['total_latency_real_us'] = df['t1_t4_us']         # t1→t4
        ...
        stats = {
            ...
            # CPU 사용량
            'cpu_total_mean': df['cpu_total'].mean(),
            'cpu_gz_mean': df['cpu_gz'].mean(),
            'cpu_px4_mean': df['cpu_px4'].mean(),
            'cpu_mav_mean': df['cpu_mav'].mean(),
        }
        ...
        for node_id in df['node_id'].unique():
            ...
            by_topic[node_id] = {
                ...
                # CPU 사용량
                'cpu_total_mean': topic_df['cpu_total'].mean(),
                'cpu_gz_mean': topic_df['cpu_gz'].mean(),
                'cpu_px4_mean': topic_df['cpu_px4'].mean(),
                'cpu_mav_mean': topic_df['cpu_mav'].mean(),
            }
...
def print_statistics(stats_list):
    ...
    for stats in stats_list:
        ...
        print(f"\n   💻 CPU 사용량 (평균):")
        print(f"      시스템 전체: {stats['cpu_total_mean']:.1f}%")
        print(f"      Gazebo: {stats['cpu_gz_mean']:.1f}%, PX4: {stats['cpu_px4_mean']:.1f}%, MAVROS: {stats['cpu_mav_mean']:.1f}%")
        ...
        if stats['by_topic']:
            ...
            for node_id in sorted(stats['by_topic'].keys()):
                ...
                print(f"         CPU: 전체={topic_stats['cpu_total_mean']:.1f}%, GZ={topic_stats['cpu_gz_mean']:.1f}%, PX4={topic_stats['cpu_px4_mean']:.1f}%, MAV={topic_stats['cpu_mav_mean']:.1f}%")
...
def save_analysis(stats_list, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        for stats in stats_list:
            ...
            f.write(f"CPU 사용량 (평균):\n")
            f.write(f"  시스템 전체: {stats['cpu_total_mean']:.1f}%\n")
            f.write(f"  Gazebo: {stats['cpu_gz_mean']:.1f}%, PX4: {stats['cpu_px4_mean']:.1f}%, MAVROS: {stats['cpu_mav_mean']:.1f}%\n")
            ...
            if stats['by_topic']:
                ...
                for node_id in sorted(stats['by_topic'].keys()):
                    ...
                    f.write(f"    CPU: 전체={topic_stats['cpu_total_mean']:.1f}%, GZ={topic_stats['cpu_gz_mean']:.1f}%, PX4={topic_stats['cpu_px4_mean']:.1f}%, MAV={topic_stats['cpu_mav_mean']:.1f}%\n")
            f.write("\n")
