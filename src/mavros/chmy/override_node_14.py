...
class OverrideNode14(Node):
    def __init__(self):
        super().__init__("override_node_14")
        self.publisher = self.create_publisher(OverrideRCIn, "/mavros/rc/override", 10)
        frequency = get_node_frequency("override_node_14")
        self.timer = self.create_timer(frequency, self.timer_callback)
        self.get_logger().info(f"Override Node 14 started at {int(1/frequency):,}Hz")
